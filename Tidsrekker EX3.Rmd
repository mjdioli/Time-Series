...
```{r}
sarima(ts, p=1, d=2, q=1, P=1, Q=1, D=1, S=12)
#test
'?'(sarima)
```
...
for(p1 in 4:5){
  for(q1 in 1:5){
    for(P1 in 1:5){
      for(Q1 in 1:5){
        mod <- tryCatch(sarima(ts, p=p1, d=2, q=q1, P=P1, D=1, S=12, Q = Q1, no.constant = T, tol = 1e-3),
...
```{r}
mod <- sarima(ts, p=4, d=2, q=1, P=5, Q=1, D=1, S=12)
mod$ttable
```
...
acf2(diff(ts_daily_insitu))


'?'(acf)
#Second difference
acf2(diff(diff(ts_daily_flask)))
acf2(diff(diff(ts_daily_insitu)))

#Single differencing seems better than double
```
