...
$\phi_1=$`r mod$fit$coef[1]`, $\phi_2=$`r mod$fit$coef[2]`, $\phi_3=$`r mod$fit$coef[3]`,$\phi_4=$`r mod$fit$coef[4]`, $\theta_1=$`r mod$fit$coef[5]`, $\Phi_1=$`r mod$fit$coef[6]`, $\phi_2=$`r mod$fit$coef[7]`, $\Phi_3=$`r mod$fit$coef[8]`,$\Phi_4=$`r mod$fit$coef[9]`,$\Phi_5=$`r mod$fit$coef[10]`, $\Theta_1=$`r mod$fit$coef[11]`
...
We can also plot the model fit, versus the real values, getting the following:
```{r}
...
For the coefficients we get the following bootstrapped confidence intervals (95%):
```{r include=FALSE}
...
# Discussion
The model seem to predict an increase of the co2 levels in the next year. And seem to be quite confident on its predictions.
